...
const Dice = () => {
  ...
  const [activeServerSeed, setActiveServerSeed] = useState(11);
  ...
  const myRulesNextRoll = (lastResult) => {
    ...
    //next bet is 0.005747126437 of profit
    const percentage  = 0.005747126437 //%;
    const percentageDivided = percentage / 100;
    const multi = percentageDivided * profit;
    const nextBetAmount = multi < 0.00000005 ? 0.00000005 : Number(multi.toFixed(8));
    //
    ...
    if(!lastResult.win){
      ...
      if(byThreeTemp == 3 || byThreeTemp == 5){
        setBetAmount(0.00000005);
      }
      else if(byThreeTemp >= 11){
        if(!flag){
          setFlag(true);
          // setBetAmount(betAmount => betAmount * 9);
    ...
    // comment dito
    // nag end ako sa index 10 activeServer, at lahat mababa value pero puro positive.
    // icontinue sa 11 para maobserbahan pa
