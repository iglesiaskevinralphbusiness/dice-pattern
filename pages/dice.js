...
const Dice = () => {
  ...
  const [balance, setBalance] = useState(0.0025);
  ...
  const [activeServerSeed, setActiveServerSeed] = useState(7);
  ...
  const myRulesNextRoll = useCallback((lastResult) => {
    ...
    if(condition == 0){
      ...
      if(loseStreak == 30){
        setChance(chance => Number(Number(chance + 0.4).toFixed(2)));
        setCondition(3);
      }
